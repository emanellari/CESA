...
def infer_column_type(series: pd.Series) -> str:
    ...
    unique_vals = [str(v).strip().lower() for v in s_str.unique()]
    ...
    if len(unique_vals) == 2 and set(unique_vals) in boolean_pairs:
...
def render_datasets_page():
    require_login()
    st.markdown("<h1 style='text-align: center;'>Datasets</h1><hr style='margin-top: -10px; margin-bottom: 30px;'>", unsafe_allow_html=True)
    ...
    if datasets:
        ...
        options = {f"{d['name']}  (ID: {d['dataset_id']})": d["dataset_id"] for d in datasets}
        label = st.selectbox("Select a dataset", list(options.keys()))
        ...
        with col1:
            if st.button("Open Dataset", use_container_width=True):
                with st.spinner("Loading dataset..."):
                    r = load_dataset_into_session(selected_id)
                if r.ok:
        ...
        with col2:
            if st.button("Delete Dataset", use_container_width=True):
                ...
                if r.ok:
                    ...
                    show_http_error(r)
        with col3:
            if st.button("Create Dataset", use_container_width=True, key="create_dataset_top"):
                with st.spinner("Loading..."):
                    st.session_state.page = "Crear desde 0"
                    st.rerun()
    else:
        st.markdown("<div style='background-color: #f9f9f9; padding: 20px; border-radius: 10px; text-align: center; color: #555;'><h4>No datasets available</h4><p>You have not uploaded any datasets yet.</p></div>", unsafe_allow_html=True)

    st.divider()
    if st.button("Create Dataset", use_container_width=True, key="create_dataset_bottom"):
        with st.spinner("Loading..."):
            st.session_state.page = "Crear desde 0"
            st.rerun()

    st.markdown('<h2 style="text-align: center;">OR</h2><h2 style="text-align: center;">↓</h2>', unsafe_allow_html=True)
    st.divider()
    st.markdown("### Upload New Dataset")
    uploaded = st.file_uploader(
        "Drag and select a dataset file",
    ...
    if uploaded is not None:
        ...
        st.success("File loaded locally. Review the dataset before uploading.")
        st.dataframe(df_uploaded.head(20), use_container_width=True)
        st.session_state.upload_preview_df = df_uploaded.copy()

        if "upload_profiles" not in st.session_state or st.session_state.get("last_uploaded_name") != uploaded.name:
        ...
        with col_b:
            if st.button("Upload cleaned dataset", use_container_width=True):
                try:
                    ...
                    if r.ok:
                        payload = r.json()
                        st.write("UPLOAD META:", payload.get("meta"))
